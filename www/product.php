<?php
    session_start();
        ...
        $file = fopen('admin/items.db', 'r');
        $new = true;
        while(!feof($file)){
            $line = fgets($file);
            $array = explode(";",$line);
            if(trim($array[0]) == $_SESSION["clickedname"]){
                $_POST["name"] = trim($array[0]);
                $_POST["price"] = trim($array[1]);
                $_POST["pimage"] = trim($array[2]);
                $_POST["description"] = trim($array[3]); 
                $_POST["status"] = trim($array[4]);
                break;
        }
    }
    if(isset($_GET['buy'])){
        ...
        foreach($_SESSION['items'] as $item){
            if($item[0] == $_POST['name']){
                $new = false;
                $_SESSION['items'][$i][5] += 1;
                $_SESSION['items'][$i][6] = intval($_SESSION['items'][$i][1]) * $_SESSION['items'][$i][5];
        ...
        if($new == true){
        array_push($_SESSION["items"], array($_POST["name"] , $_POST["price"] , $_POST["pimage"] , $_POST["description"], $_POST["status"], 1, $_POST["price"] ));
        header("location: cart.php");
        }
    }
    ?>
...
echo "<div id='product-head' class='row'>
<div id='product-img' class='col-lg-6 col-md-6 col-sm-12'>
    <img id ='prodimg' src=' "; echo$_POST["pimage"]; echo" '>
</div>
<div id='product-details' class='col-lg-6 col-md-6 col-sm-12'>
    <h1>"; echo $_POST["name"]; echo "</h1>
    <ul>
        <li id='price'>Price (not include VAT)</li>
        <li id='price-number'>$"; echo $_POST["price"];echo "</li>
        <li id='status'>"; echo$_POST["status"]; echo"</li>
    </ul>
    <div id='add-cart'><a href='?buy=yes'>Buy Now</a></div>
</div>
</div>
<div id='product-body' class='row'>
<div class='col-lg-12 col-md-12 col-sm-12'>
    <h3>About &nbsp"; echo $_POST["name"]; echo"</h3>
    <p>"; echo $_POST["description"]; echo"</p>
</div>
</div>
";
                    	...
                    	<div class="col-lg-12 col-md-12 col-sm-12">
                            <div class="comment-item">
                                <ul>
                                    ...
                                    <li>
                                        <p>Beautiful design, very sensitive touch, no need to hold in hand. Take relatively sharp photos, play the game very well. If the price is a bit soft, it will sell well. A good product that everyone can consider.</p>
                                    </li>
                                </ul>
                            </div>
                            <div class="comment-item">
