        ...
        <div class="info">
            <fieldset>
                <form method= "post" enctype="multipart/form-data">
                    ...
                    <input type="text" class = 'info_input' name="b_address" id="business_address">
                    <label for="image">Set a profile picture: </label>
                    <input type="file" name="avt" id="image" accept="image/png, image/jpeg">
                    <?php
                            error_reporting(E_ERROR | E_PARSE);

                            $tmp_avt = $_FILES["avt"]["tmp_name"];

                            // error_reporting(E_ERROR | E_PARSE);
                            if(isset($_POST["act"]))
                            {
                                if(isset($_POST["username"]) && isset($_POST["password"]) && isset($_POST["b_address"]) && isset($_POST["b_name"]) && isset($_FILES['avt']['tmp_name']) )
                                {
                                    $userptn = '/^(?=.*\d)(?=.*[a-z])(?!\W+)(?=.*[A-Z])(?!.*\s).{8,15}$/';
                                    $pwdptn = '/^(?=.*\d)(?=.*[a-z])(?=.*[!@#\$%\^\&*])(?=.*[A-Z])(?!.*\s).{8,20}$/';
                                    $addptn = '/^.{5,}$/';
                                    if(preg_match($userptn , $_POST["username"]) and preg_match($pwdptn , $_POST["password"]) and preg_match($addptn , $_POST["b_address"]) and preg_match($addptn , $_POST["b_name"]))
                                    {
                                    ...
                                    // check if user exist.
                                        ...
                                        else
                                        {
                                            ...
                                            $avt = $_FILES["avt"];
                                            move_uploaded_file($_FILES["avt"]["tmp_name"], "avtimg/".$_POST["username"]."avt.png");
                                        ...
                                        echo "Please input in the correct  format!";
                                    }
                                }
                                else{
